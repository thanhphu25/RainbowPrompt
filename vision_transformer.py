...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        logger.info(f'Use prefix tune for g prompt: {self.use_prefix_tune_for_g_prompt}')
        logger.info(f'G prompt layer index: {self.g_prompt_layer_idx}')
        logger.info(f'Num g prompt: {num_g_prompt}')
        logger.info(f'G prompt: {self.g_prompt}')

        if use_e_prompt and e_prompt_layer_idx is not None:
            self.rainbow_prompt = RainbowPrompt(length=prompt_length, embed_dim=embed_dim, embedding_key=embedding_key, prompt_init=prompt_init,
